...
- **Concurrency:** one `pg_advisory_xact_lock` per assignment transaction. This is the load-bearing correctness mechanism for multi-BDC-agent races. Every path that changes ordering (assign, void, reassign, status override, reactivation) takes the same lock. A bulk status change takes it **once for the whole batch**, not once per rep — `bulkOverrideStatus` opens one transaction and calls `applyOverrideStatus` per rep inside it. Per-rep transactions would make a partial apply reachable. A rep whose visible status already matches the action is skipped, using the one shared `isOverrideNoOp` in `packages/core`; do not add a second copy of that rule to the client.
...
- **Disqualification is applied by a manager click, not by a job.** The activity import is the gate: after the CSV is parsed and matched, the manager sees a preview of who would be deactivated and picks one of three outcomes — *log numbers & deactivate N*, *log numbers only*, or *cancel the entire import*. Log-only and deactivate both write `rep_daily_activity` and the `eligibility_snapshot` rows, so the calculation is on record either way; only the deactivate branch writes `rep_daily_status` (suspension runs through Saturday). Cancel writes nothing at all — preview is side-effect-free by design. This human gate replaces the SHADOW → ENFORCE rollout: the thresholds still need calibration, and a manager reviewing each day's list is a better calibration loop than a policy flag nobody can see. Do not add an automatic deactivation path that skips this prompt.
  ...
  - The commit path is `apps/api/src/jobs/activityImportDecision.ts`. It re-derives the preview inside the transaction and rejects a stale decision via an HMAC preview token — a manager cannot apply a "Yes" to numbers that changed after they reviewed them.
  - A manager deactivation remains until a manager reactivates the rep. A manager activation is not an activity exemption: failing reviewed/imported activity may replace it with `SYSTEM` `INELIGIBLE` through Saturday. Passing activity never auto-reactivates an inactive rep.
...
- `pnpm --filter @phoneup/api rotate-passwords [--commit]` — one-off remediation; dry-run by default. Deliberately **skips** accounts already flagged `mustChangePassword`, so it cannot rescue a never-used admin — that is what `recover-admin` is for.
- `pnpm --filter @phoneup/api recover-admin [email] [--commit]` — break-glass for a locked-out ADMIN. Dry-run lists the ADMIN accounts and what it would do. Issues a fresh temp password through `resetPassword` (same generator, sessions revoked, `must_change_password` set, audit-logged), and reactivates the account if it was inactive. Requires naming the email when more than one ADMIN exists. ADMIN accounts only.
